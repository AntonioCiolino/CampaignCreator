...
class LocalLLMService(AbstractLLMService):
    ...
    async def generate_campaign_concept(self, user_prompt: str, db: Session, current_user: UserModel, model: Optional[str] = None) -> str: # Added current_user
        ...
        return await self.generate_text(prompt=final_prompt, _current_user=current_user, db=db, model=model) # Pass args
    ...
    async def generate_titles(self, campaign_concept: str, db: Session, current_user: UserModel, count: int = 5, model: Optional[str] = None) -> list[str]: # Added current_user
        ...
        generated_string = await self.generate_text(prompt=final_prompt, _current_user=current_user, db=db, model=model) # Pass args
    ...
    async def generate_section_content(
        ...
        return await self.generate_text(prompt=final_prompt_for_generation, _current_user=current_user, db=db, model=model, temperature=0.7, max_tokens=4000) # Pass args
